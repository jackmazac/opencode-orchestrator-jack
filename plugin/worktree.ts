...
async function forkWithContext(
  ...
  let destWorkspaceDir = "";
  let destDelegationsDir = "";
  let destPlan = "";
  ...
  try {
    ...
    destWorkspaceDir = path.join(workspaceBase, projectId, forkedSession.id);
    destDelegationsDir = path.join(delegationsBase, projectId, forkedSession.id);
    ...
    destPlan = path.join(destWorkspaceDir, "plan.md");
  ...
  return {
    ...
    delegationsDir: destDelegationsDir,
    planPath: planCopied ? destPlan : undefined,
    delegationsPath: delegationsCopied ? destDelegationsDir : undefined,
  };
}
...
function formatPathSection(baseDir: string, title: string, filePaths: string[]): string {
  if (filePaths.length === 0) return "";
  return `\n${title}:\n${filePaths.map((filePath) => rel(baseDir, filePath)).join("\n")}`;
}

async function getWorktreeStateDbPath(projectRoot: string): Promise<string> {
      ...
      worktree_create: tool({
        ...
        async execute(args, toolCtx) {
          ...
          const {
            ...
            planPath,
            delegationsPath,
          } = await forkWithContext(client, toolCtx.sessionID, projectId, async (sid) => {
          ...
          if (!terminalResult.success) {
            return `❌ Failed to launch worktree terminal: ${terminalResult.error ?? "unknown error"}\nWorktree created at ${worktreePath}. Verify launch settings and retry.\nworktree: ${rel(directory, worktreePath)}`;
          }

          const paths = [
            `worktree: ${rel(directory, worktreePath)}`,
            `state db: ${rel(directory, await getWorktreeStateDbPath(directory))}`,
            ...(configExisted ? [] : [`config: ${rel(directory, configPath)}`]),
            `session workspace: ${rel(directory, workspaceDir)}`,
            `session delegations: ${rel(directory, delegationsDir)}`,
            ...(planCopied && planPath ? [`copied plan: ${rel(directory, planPath)}`] : []),
            ...(delegationsCopied && delegationsPath
              ? [`copied delegations: ${rel(directory, delegationsPath)}`]
              : []),
          ].join("\n");

          return `Worktree created at ${worktreePath}\nrelative paths:\n${paths}${formatPathSection(directory, "copied files", copiedFiles)}${formatPathSection(directory, "symlinked directories", symlinkedDirs)}\n\nA new terminal has been opened with OpenCode.`;
        },
      }),
