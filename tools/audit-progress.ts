...
export const update = tool({
  ...
  args: {
    audit_slug: tool.schema
      .string()
      .describe("Audit slug this progress tracks"),
    wave_id: tool.schema
      .string()
      .describe("Wave identifier (e.g. W1, explore-batch-a)"),
    status: tool.schema
      .string()
    ...
    summary: tool.schema
      ...
      .optional()
      .describe(
        "Brief summary (e.g. '3 explore tasks returned', synthesis done)",
      ),
  },
...
export const read = tool({
  ...
  args: {
    audit_slug: tool.schema
      ...
      .optional()
      .describe(
        "Audit slug to read. Omit to list all audits with progress summaries.",
      ),
  },
  async execute(args, context) {
    if (args.audit_slug) {
      ...
      if (!fs.existsSync(dest))
        return `no audit progress for ${args.audit_slug}`
...
export const done = tool({
  ...
  args: {
    audit_slug: tool.schema
      ...
      .optional()
      .describe(
        "Audit slug to remove. Omit to remove ALL audit progress files.",
      ),
  },
  async execute(args, context) {
    if (args.audit_slug) {
